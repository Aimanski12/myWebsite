...
function Footer(props) {
 ...
 const navs = () => {
    let a = props.navs
    let c = a.nav.map((b, i)=>{
        return (<Link key={i} href={b === 'home' ? '/' : `/${b}`}
                    // onClick={()=> props.click(b)}
                    ><a>{b.toUpperCase()}</a></Link>)
      })
    return c
  }
    ...
    <footer className="footer content-center">
      ...
      <div className="footer-nav-container content-align content-center">
        ...
        <div className="footer-blocks content-center content-align">
          <p>&copy; <span className='green'>Aiman Adlawan</span>. All rights reserved. <Link href="/privacy-policy">
            <a target='_blank'><span className='orange'>Privacy Policy</span></a></Link>.</p>
